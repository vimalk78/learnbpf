...
static int matchPrefix(const char str[MAX_FILE_NAME_LENGTH]);
...
int syscall_enter_open(struct syscall_enter_openat_args_t *args) {
  ...
  bpf_printk("syscall_enter_open file: %s", args->filename);

  u64 pid_tgid;
  pid_tgid = bpf_get_current_pid_tgid();

  long ret =
      bpf_map_update_elem(&ctx_syscall_open, &pid_tgid, filename, BPF_ANY);
  return ret;
}
...
int syscall_enter_write(struct syscall_enter_write_args_t *args) {
  ...
  // print path,count
  bpf_printk("write called for file: %s, num: %d", path, args->count);
  return 0;
}
...
static inline int matchPrefix(const char str[MAX_FILE_NAME_LENGTH]) {
  if (str == NULL) {
    return -100;
  }
  for (__u16 i = 0; i < LEN_LOG_DIR; i++) {
    const char ch1 = LOG_DIR[i];
    if (ch1 == '\0') {
      return 0;
    }
    const char ch2 = str[i];
    if (ch2 == '\0') {
      return -1;
    }
    if (ch1 != ch2) {
...
static inline int matchTrie(const char *str) { return 0; }
